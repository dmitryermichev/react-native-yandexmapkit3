...
class RNYandexPlacemark: UIView {
    @objc var onStatusChange: RCTDirectEventBlock?
    @objc var isOn: Bool = false  {
        didSet {
            button.setTitle(String(describing: isOn ? "Switch Off" : "Switch On"),
                            for: .normal)
...
//        isOn = !isOn as BoolonStatusChange?;(["isOn": isOn])
    }
